...
if args.type_haystack == 'essay':
    essay = os.path.join("synthetic/PaulGrahamEssays.json")
...
def generate_input_output(num_haystack):
    ...
    answer_prefix = args.answer_prefix.format(
        type_needle_v=type_needle_v,
        query=query,
    )
    return input_text, context, answer_prefix, answers
...
def generate_samples(num_samples: int, max_seq_length: int, incremental: int = 500):
    ...
    while total_tokens + tokens_to_generate < max_seq_length :  
        input_text, context, answer_prefix, answer = generate_input_output(num_haystack)
        # Calculate the number of tokens in the example
        total_tokens = len(TOKENIZER.text_to_tokens(input_text + ' '.join(answer)+ f' {context}'))
    ...
    for index in range(num_samples):
        ...
        while(True):
            try:
                input_text, context, answer_prefix, answer  = generate_input_output(used_haystack)
