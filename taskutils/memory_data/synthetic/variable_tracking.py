...
def generate_input_output(num_noises, num_chains, num_hops, is_icl=False):
    ...
    asnwer_prefix = args.answer_prefix.format(
        query=value,
        num_v=num_hops+1
    )
    return input_text, context, asnwer_prefix, vars[0]
...
def sys_vartrack_w_noise_random(num_samples: int, max_seq_length: int, incremental: int = 10, 
    ...
    while total_tokens + tokens_to_generate + example_tokens < max_seq_length :
        input_text, context, answer_prefix, answer = generate_input_output(num_noises, num_chains, num_hops, is_icl=add_fewshot & (icl_example is None))
    ...
    for index in range(num_samples):
        ...
        while(True):
            try:
                input_text, context, answer_prefix, answer = generate_input_output(used_noises, num_chains, num_hops, is_icl=add_fewshot & (icl_example is None))
                length = len(TOKENIZER.text_to_tokens(input_text+f'{context}')) + tokens_to_generate + example_tokens
