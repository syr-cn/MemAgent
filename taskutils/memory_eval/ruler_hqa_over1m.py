...
import sys
sys.path.append("../memory_data")
from processing import read_hotpotqa
DOCS = None
...
def main():
    ...
    items = [set_context(item) for item in dataset]
    print(f"original data len {len(dataset)}")
    # 通过深拷贝生成新数据集
    import copy
    dataset = [copy.deepcopy(item) for _ in range(args.sampling) for item in items]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--length", type=int, default=12800, choices=[12800, 25600, 51200, 66635])
